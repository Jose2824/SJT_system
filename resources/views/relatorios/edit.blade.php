...
<div class="max-w-2xl mx-auto bg-white shadow-lg rounded-2xl p-8 mt-8 border border-[#BFC4C9]">
    <h1 class="text-2xl font-bold text-[#001C91] mb-6 text-center">Editar Relatório de Custo</h1>
    ...
    <form action="{{ route('relatorios.update', $relatorio->id) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Veículo (Placa)</label>
            <input type="text" name="veiculo" value="{{ old('veiculo', $relatorio->veiculo) }}" 
        ...
        <div>
            ...
            <textarea name="descricao" 
                class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#0026FF]" 
                placeholder="Descreva o gasto">{{ old('descricao', $relatorio->descricao) }}</textarea>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Valor (R$)</label>
            <input type="text" name="valor" value="{{ old('valor', $relatorio->valor) }}" 
                class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#0026FF]" 
        ...
        <div>
            ...
            <input type="date" name="data" value="{{ old('data', $relatorio->data) }}" 
                class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-[#0026FF]">
        </div>
